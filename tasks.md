...
- [x] **Submit 탭**: 제출 가이드 텍스트 표시
- [x] 단계별 제출 폼 구현 및 유효성 검사
  - [x] 메모(`notes`) — 텍스트, 선택
  - [x] 기획서 (1차) — text 또는 URL
  - [x] 최종 웹링크 (2차) — URL
  - [x] 솔루션 PDF (3차) — PDF URL
  - [x] 파일 (zip·pdf·csv 등, 해커톤 설정에 따라 허용 형식 다름)
- [x] [예외] 필수 항목 미입력 시 인라인 에러 메시지 노출 및 "제출" 버튼 비활성화
- [x] "제출" 클릭 시 localStorage `submissions` 저장 및 제출 완료 알림
- [x] **Leaderboard 탭**: `leaderboards[slug]` + 제출 데이터 join하여 순위 테이블 렌더링 (rank·팀명·score·제출일)
- [x] [예외] 미제출 팀: "미제출" 배지 표시 및 순위 제외
- [x] 제출 완료 시 리더보드 상태 자동 갱신 ("미제출" → "제출 완료")
